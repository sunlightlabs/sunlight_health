...
boxShadow:!!c("boxShadow")&&!i,scrollTop:("pageXOffset"in window||"scrollTop"in document.documentElement||"scrollTop"in f[0])&&!e,dynamicBaseTag:function(){var h=location.protocol+"//"+location.host+location.pathname+"ui-dir/",k=a("head base"),j=null,p="";if(k.length)p=k.attr("href");else k=j=a("<base>",{href:h}).appendTo("head");var q=a("<a href='testurl'></a>").prependTo(f)[0].href;k[0].href=p?p:location.pathname;j&&j.remove();return q.indexOf(h)===0}()});f.remove();a.support.boxShadow||a("html").addClass("ui-mobile-nosupport-boxshadow")})(jQuery);
(function(a,d){a.each("touchstart touchmove touchend orientationchange tap taphold swipe swipeleft swiperight scrollstart scrollstop".split(" "),function(e,i){a.fn[i]=function(h){return h?this.bind(i,h):this.trigger(i)};a.attrFn[i]=true});var c=a.support.touch,f=c?"touchstart":"mousedown",b=c?"touchend":"mouseup",g=c?"touchmove":"mousemove";a.event.special.scrollstart={enabled:true,setup:function(){function e(j,p){h=p;var q=j.type;j.type=h?"scrollstart":"scrollstop";a.event.handle.call(i,j);j.type=
q}var i=this,h,k;a(i).bind("touchmove scroll",function(j){if(a.event.special.scrollstart.enabled){h||e(j,true);clearTimeout(k);k=setTimeout(function(){e(j,false)},50)}})}};a.event.special.tap={setup:function(){var e=this,i=a(e);i.bind("mousedown touchstart",function(h){function k(o){if(o.type=="scroll")j=true;else{o=o.type=="touchmove"?o.originalEvent.touches[0]:o;if(Math.abs(w[0]-o.pageX)>10||Math.abs(w[1]-o.pageY)>10)j=true}}if(h.which&&h.which!==1||i.data("prevEvent")&&i.data("prevEvent")!==h.type)return false;
i.data("prevEvent",h.type);setTimeout(function(){i.removeData("prevEvent")},800);var j=false,p=true,q=h.target,s=h.originalEvent,w=h.type=="touchstart"?[s.touches[0].pageX,s.touches[0].pageY]:[h.pageX,h.pageY],m,r;r=setTimeout(function(){if(p&&!j){m=h.type;h.type="taphold";a.event.handle.call(e,h);h.type=m}},750);a(window).one("scroll",k);i.bind("mousemove touchmove",k).one("mouseup touchend",function(o){i.unbind("mousemove touchmove",k);a(window).unbind("scroll",k);clearTimeout(r);p=false;if(!j&&
q==o.target){m=o.type;o.type="tap";a.event.handle.call(e,o);o.type=m}})})}};a.event.special.swipe={setup:function(){var e=a(this);e.bind(f,function(i){function h(q){if(j){var s=q.originalEvent.touches?q.originalEvent.touches[0]:q;p={time:(new Date).getTime(),coords:[s.pageX,s.pageY]};Math.abs(j.coords[0]-p.coords[0])>10&&q.preventDefault()}}var k=i.originalEvent.touches?i.originalEvent.touches[0]:i,j={time:(new Date).getTime(),coords:[k.pageX,k.pageY],origin:a(i.target)},p;e.bind(g,h).one(b,function(){e.unbind(g,
h);if(j&&p)if(p.time-j.time<1E3&&Math.abs(j.coords[0]-p.coords[0])>30&&Math.abs(j.coords[1]-p.coords[1])<75)j.origin.trigger("swipe").trigger(j.coords[0]>p.coords[0]?"swipeleft":"swiperight");j=p=d})})}};(function(e){function i(){var p=k();if(p!==j){j=p;h.trigger("orientationchange")}}var h=e(window),k,j;e.event.special.orientationchange={setup:function(){if(e.support.orientation)return false;j=k();h.bind("resize",i)},teardown:function(){if(e.support.orientation)return false;h.unbind("resize",i)},
add:function(p){var q=p.handler;p.handler=function(s){s.orientation=k();return q.apply(this,arguments)}}};k=function(){var p=document.documentElement;return p&&p.clientWidth/p.clientHeight<1.1?"portrait":"landscape"}})(jQuery);a.each({scrollstop:"scrollstart",taphold:"tap",swipeleft:"swipe",swiperight:"swipe"},function(e,i){a.event.special[e]={setup:function(){a(this).bind(i,a.noop)}}})})(jQuery);
(function(a,d,c){function f(j){j=j||location.href;return"#"+j.replace(/^[^#]*#?(.*)$/,"$1")}var b="hashchange",g=document,e,i=a.event.special,h=g.documentMode,k="on"+b in d&&(h===c||h>7);a.fn[b]=function(j){return j?this.bind(b,j):this.trigger(b)};a.fn[b].delay=50;i[b]=a.extend(i[b],{setup:function(){if(k)return false;a(e.start)},teardown:function(){if(k)return false;a(e.stop)}});e=function(){function j(){var o=f(),t=r(s);if(o!==s){m(s=o,t);a(d).trigger(b)}else if(t!==s)location.href=location.href.replace(/#.*/,
"")+t;q=setTimeout(j,a.fn[b].delay)}var p={},q,s=f(),w=function(o){return o},m=w,r=w;p.start=function(){q||j()};p.stop=function(){q&&clearTimeout(q);q=c};a.browser.msie&&!k&&function(){var o,t;p.start=function(){if(!o){t=(t=a.fn[b].src)&&t+f();o=a('<iframe tabindex="-1" title="empty"/>').hide().one("load",function(){t||m(f());j()}).attr("src",t||"javascript:0").insertAfter("body")[0].contentWindow;g.onpropertychange=function(){try{if(event.propertyName==="title")o.document.title=g.title}catch(v){}}}};
p.stop=w;r=function(){return f(o.location.href)};m=function(v,l){var n=o.document,u=a.fn[b].domain;if(v!==l){n.title=g.title;n.open();u&&n.write('<script>document.domain="'+u+'"<\/script>');n.close();o.location.hash=v}}}();return p}()})(jQuery,this);
...
1]},addNew:function(l,n){h.getNext()&&h.clearForward();h.stack.push({url:l,transition:n});h.activeIndex=h.stack.length-1},clearForward:function(){h.stack=h.stack.slice(0,h.activeIndex+1)},ignoreNextHashChange:true},k="[tabindex],a,button:visible,select:visible,input",j=null,p=[],q=false,s="&ui-state=dialog",w=g.children("base"),m=location.protocol+"//"+location.host,r=e.get(m+location.pathname),o=r;if(w.length){var t=w.attr("href");if(t)o=t.search(/^[^:\/]+:\/\/[^\/]+\/?/)===-1?t.charAt(0)==="/"?
m+t:r+t:t;o+=o.charAt(o.length-1)==="/"?" ":"/"}var v=a.support.dynamicBaseTag?{element:w.length?w:a("<base>",{href:o}).prependTo(g),set:function(l){v.element.attr("href",o+e.get(l))},reset:function(){v.element.attr("href",o)}}:d;e.setOrigin();a.fn.animationComplete=function(l){if(a.support.cssTransitions)return a(this).one("webkitAnimationEnd",l);else{setTimeout(l,0);return a(this)}};a.mobile.updateHash=e.set;a.mobile.path=e;a.mobile.base=v;a.mobile.urlstack=h.stack;a.mobile.urlHistory=h;a.mobile.changePage=
function(l,n,u,z,F){function J(){q=false;p.length>0&&a.mobile.changePage.apply(a.mobile,p.pop())}function E(){function B(){if(z!==false&&x){h.ignoreNextHashChange=false;e.set(x)}!N&&!O&&h.addNew(x,n);c();a.mobile.silentScroll(l.data("lastScroll"));var C=l,G=C.find(".ui-title:eq(0)");G.length?G.focus():C.find(k).eq(0).focus();y&&y.data("page")._trigger("hide",null,{nextPage:l});l.data("page")._trigger("show",null,{prevPage:y||a("")});a.mobile.activePage=l;P!==null&&P.remove();b.removeClass("ui-mobile-rendering");
J()}function K(C){a.mobile.pageContainer.addClass(C);L.push(C)}a.mobile.silentScroll();var H=f.scrollTop(),Q=["flip"],L=[];if(x.indexOf("&"+a.mobile.subPageUrlKey)>-1)l=a("[data-url='"+x+"']");if(y){y.data("lastScroll",H);y.data("page")._trigger("beforehide",{nextPage:l})}l.data("page")._trigger("beforeshow",{prevPage:y||a("")});if(n&&n!=="none"){a.mobile.pageLoading(true);a.inArray(n,Q)>=0&&K("ui-mobile-viewport-perspective");K("ui-mobile-viewport-transitioning");if(y)y.addClass(n+" out "+(u?"reverse":
""));l.addClass(a.mobile.activePageClass+" "+n+" in "+(u?"reverse":""));l.animationComplete(function(){l.add(y).removeClass("out in reverse "+n);y&&y.removeClass(a.mobile.activePageClass);B();a.mobile.pageContainer.removeClass(L.join(" "));L=[]})}else{a.mobile.pageLoading(true);y&&y.removeClass(a.mobile.activePageClass);l.addClass(a.mobile.activePageClass);B()}}function U(){if(j||l.data("role")==="dialog"){x=h.getActive().url+s;if(j){l.attr("data-role",j);j=null}}l.page()}var I=a.type(l)==="array",
M=a.type(l)==="object",y=I?l[0]:a.mobile.activePage;l=I?l[1]:l;var x=a.type(l)==="string"?e.stripHash(l):"",A=x,D,R="get",V=false,P=null,S=h.getActive(),N=false,O=false;if(!(S&&h.stack.length>1&&S.url===x&&!I&&!M))if(q)p.unshift(arguments);else{q=true;if(F){var T;a.each(h.stack,function(B){if(this.url===x){N=B<h.activeIndex;O=!N;T=B}});h.activeIndex=T?T:h.activeIndex;if(N){u=true;n=n||S.transition}else if(O)n=n||h.getActive().transition}if(M&&l.url){x=l.url;D=l.data;R=l.type;V=true;if(D&&R==="get"){if(a.type(D)===
"object")D=a.param(D);x+="?"+D;D=d}}v&&v.reset();a(window.document.activeElement).add("input:focus, textarea:focus, select:focus").blur();if(x){l=a("[data-url='"+x+"']");A=e.getFilePath(x)}else{I=l.attr("data-url");M=e.getFilePath(I);if(I!==M)A=M}if(n===d)n=j&&j==="dialog"?"pop":a.mobile.defaultTransition;if(l.length&&!V){A&&v&&v.set(A);U();E()}else{if(l.length)P=l;a.mobile.pageLoading();a.ajax({url:A,type:R,data:D,dataType:"html",success:function(B){var K=a("<div></div>"),H,Q=/\bdata-url=["']?([^"'>]*)["']?/;
if(/.*(<[^>]*\bdata-role=["']?page["']?[^>]*>).*/.test(B)&&RegExp.$1&&Q.test(RegExp.$1)&&RegExp.$1)H=RegExp.$1;if(H){v&&v.set(H);x=A=e.getFilePath(H)}else v&&v.set(A);K.get(0).innerHTML=B;l=K.find('[data-role="page"], [data-role="dialog"]').first();if(!a.support.dynamicBaseTag){var L=e.get(A);l.find("[src],link[href]").each(function(){var C=a(this).is("[href]")?"href":"src",G=a(this).attr(C);G.replace(location.protocol+"//"+location.host+location.pathname,"");/^(\w+:|#|\/)/.test(G)||a(this).attr(C,
L+G)})}l.attr("data-url",A).appendTo(a.mobile.pageContainer);U();setTimeout(function(){E()},0)},error:function(){a.mobile.pageLoading(true);c(true);v&&v.set(e.get());a("<div class='ui-loader ui-overlay-shadow ui-body-e ui-corner-all'><h1>Error Loading Page</h1></div>").css({display:"block",opacity:0.96,top:a(window).scrollTop()+100}).appendTo(a.mobile.pageContainer).delay(800).fadeOut(400,function(){a(this).remove()});J()}})}}};a("form[data-ajax!='false']").live("submit",function(l){if(a.mobile.ajaxEnabled&&
a.mobile.ajaxFormsEnabled){var n=a(this).attr("method"),u=e.clean(a(this).attr("action"));if(!e.isExternal(u)){if(e.isRelative(u))u=e.makeAbsolute(u);a.mobile.changePage({url:u,type:n,data:a(this).serialize()},d,d,true);l.preventDefault()}}});a("a").live("click",function(l){var n=a(this),u=n.attr("href")||"#";u=e.clean(u);var z=n.is("[rel='external']"),F=e.isEmbeddedPage(u);z=e.isExternal(u)||z&&!F;F=n.is("[target]");var J=n.is("[data-ajax='false']");if(n.is("[data-rel='back']")){l=a.mobile.urlHistory.activeIndex;
var E;for(n=l-1;n>=0;n--){E=n;if(a.mobile.urlHistory.stack[n].url.indexOf(s)<0)break}window.history.go(E-l);return false}if(u!=="#"){i=n.closest(".ui-btn").addClass(a.mobile.activeBtnClass);if(z||J||F||!a.mobile.ajaxEnabled||!a.mobile.ajaxLinksEnabled){c(true);if(F)window.open(u);else if(J)return;else location.href=u}else{E=n.data("transition");z=(z=n.data("direction"))&&z==="reverse"||n.data("back");j=n.attr("data-rel");if(e.isRelative(u))u=e.makeAbsolute(u);u=e.stripHash(u);a.mobile.changePage(u,
E,z)}}l.preventDefault()});f.bind("hashchange",function(){var l=e.stripHash(location.hash),n=a.mobile.urlHistory.stack.length===0?false:d;if(!a.mobile.hashListeningEnabled||!h.ignoreNextHashChange||h.stack.length>1&&l.indexOf(s)>-1&&!a.mobile.activePage.is(".ui-dialog")){if(!h.ignoreNextHashChange)h.ignoreNextHashChange=true}else l?a.mobile.changePage(l,n,d,false,true):a.mobile.changePage(a.mobile.firstPage,n,true,false,true)})})(jQuery);
...
0;if(m){var o=m.offsetParent,t=document.body;for(r=m.offsetTop;m&&m!=t;){r+=m.scrollTop||0;if(m==o){r+=o.offsetTop;o=m.offsetParent}m=m.parentNode}}return r}function b(m){var r=a(window).scrollTop(),o=f(m[0]),t=m.css("top")=="auto"?0:parseFloat(m.css("top")),v=window.innerHeight,l=m.outerHeight(),n=m.parents(".ui-page:not(.ui-page-fullscreen)").length;if(m.is(".ui-header-fixed")){t=r-o+t;if(t<o)t=0;return m.css("top",n?t:r)}else{t=r+v-l-(o-t);return m.css("top",n?t:r+v-l)}}if(a.support.scrollTop){var g=
"inline",e=false,i,h,k=a.support.touch,j=k?"touchstart":"mousedown",p=k?"touchend":"mouseup",q=null,s=false,w=true;a(function(){a(document).bind(j,function(m){if(w)a(m.target).closest("a,input,textarea,select,button,label,.ui-header-fixed,.ui-footer-fixed").length||(q=g)}).bind(p,function(m){if(w)if(!a(m.target).closest("a,input,textarea,select,button,label,.ui-header-fixed,.ui-footer-fixed").length)if(!s){a.fixedToolbars.toggle(q);q=null}}).bind("scrollstart",function(m){if(!a(m.target).closest("a,input,textarea,select,button,label,.ui-header-fixed,.ui-footer-fixed").length){s=
...
h=null;if(r){h=a('.ui-footer[data-id="'+r+'"].ui-sticky-footer');if(h.length==0){h=m;m=h.clone();h.addClass("ui-sticky-footer").before(m)}m.addClass("ui-footer-duplicate");h.appendTo(a.mobile.pageContainer).css("top",0);b(h)}});a(".ui-page").live("pageshow",function(m){h&&h.length&&h.appendTo(m.target).css("top",0);a.fixedToolbars.show(true,this)});return{show:function(m,r){a.fixedToolbars.clearShowTimer();g="overlay";return(r?a(r):a.mobile.activePage?a.mobile.activePage:a(".ui-page-active")).children(".ui-header-fixed:first, .ui-footer-fixed:not(.ui-footer-duplicate):last").each(function(){var o=
a(this),t=a(window).scrollTop(),v=f(o[0]),l=window.innerHeight,n=o.outerHeight();t=o.is(".ui-header-fixed")&&t<=v+n||o.is(".ui-footer-fixed")&&v<=t+l;o.addClass("ui-fixed-overlay").removeClass("ui-fixed-inline");!t&&!m&&o.animationComplete(function(){o.removeClass("in")}).addClass("in");b(o)})},hide:function(m){g="inline";return(a.mobile.activePage?a.mobile.activePage:a(".ui-page-active")).children(".ui-header-fixed:first, .ui-footer-fixed:not(.ui-footer-duplicate):last").each(function(){var r=a(this),
...
"data-theme":q}).appendTo(o);q=a("<div>",{"class":"ui-header ui-bar-"+q}).prependTo(o);var v=a("<h1>",{"class":"ui-title"}).appendTo(q),l=a("<a>",{"data-iconpos":"notext","data-icon":"delete",text:c.closeText,href:"#","class":"ui-btn-left"}).appendTo(q).buttonMarkup()}if(i)d.buttonCount=a("<span>").addClass("ui-li-count ui-btn-up-c ui-btn-corner-all").hide().appendTo(e);c.disabled&&this.disable();f.change(function(){d.refresh()});a.extend(d,{select:f,optionElems:h,selectID:b,label:g,buttonId:k,menuId:j,
thisPage:p,button:e,menuPage:s,menuPageContent:w,screen:r,listbox:o,list:t,menuType:void 0,header:q,headerClose:l,headerTitle:v,placeholder:""});if(c.nativeMenu){f.appendTo(e).bind("touchstart mousedown",function(){e.addClass(a.mobile.activeBtnClass)}).bind("focus mouseover",function(){e.trigger("mouseover")}).bind("touchmove",function(){e.removeClass(a.mobile.activeBtnClass)}).bind("change blur mouseout",function(){e.trigger("mouseout").removeClass(a.mobile.activeBtnClass)});e.attr("tabindex","-1")}else{d.refresh();
f.attr("tabindex","-1").focus(function(){a(this).blur();e.focus()});e.bind("touchstart",function(n){a(this).data("startTouches",a.extend({},n.originalEvent.touches[0]))}).bind(a.support.touch?"touchend":"mouseup",function(n){a(this).data("moved")?a(this).removeData("moved"):d.open();n.preventDefault()}).bind("touchmove",function(n){n=n.originalEvent.touches[0];var u=a(this).data("startTouches"),z=Math.abs(n.pageY-u.pageY);if(Math.abs(n.pageX-u.pageX)>10||z>10)a(this).data("moved",true)});t.delegate("li:not(.ui-disabled, .ui-li-divider)",
"click",function(n){if(a(n.target).is("a")){var u=t.find("li:not(.ui-li-divider)").index(this);u=d.optionElems.eq(u)[0];u.selected=i?!u.selected:true;i&&a(this).find(".ui-icon").toggleClass("ui-icon-checkbox-on",u.selected).toggleClass("ui-icon-checkbox-off",!u.selected);f.trigger("change");i||d.close();n.preventDefault()}});r.add(l).add(m).bind("click",function(n){d.close();n.preventDefault();a.contains(m[0],n.target)&&n.stopImmediatePropagation()})}},_buildList:function(){var d=this,c=this.options,
f=this.placeholder,b=[],g=[],e=d.isMultiple?"checkbox-off":"false";d.list.empty().filter(".ui-listview").listview("destroy");d.select.find("option").each(function(){var i=a(this),h=i.parent(),k=i.text(),j="<a href='#'>"+k+"</a>",p=[],q=[];if(h.is("optgroup")){h=h.attr("label");if(a.inArray(h,b)===-1){g.push("<li data-role='list-divider'>"+h+"</li>");b.push(h)}}if(!this.getAttribute("value")||k.length==0||i.data("placeholder")){c.hidePlaceholderMenuItems&&p.push("ui-selectmenu-placeholder");f=d.placeholder=
k}if(this.disabled){p.push("ui-disabled");q.push("aria-disabled='true'")}g.push("<li data-icon='"+e+"' class='"+p.join(" ")+"' "+q.join(" ")+">"+j+"</li>")});d.list.html(g.join(" "));this.isMultiple||this.headerClose.hide();!this.isMultiple&&!f.length?this.header.hide():this.headerTitle.text(this.placeholder);d.list.listview()},refresh:function(d){var c=this,f=this.element,b=this.isMultiple,g=this.optionElems=f.find("option"),e=g.filter(":selected"),i=e.map(function(){return g.index(this)}).get();
...
var k=function(){return e=="input"?parseFloat(c.val()):c[0].selectedIndex},j=e=="input"?parseFloat(c.attr("min")):0,p=e=="input"?parseFloat(c.attr("max")):c.find("option").length-1,q=window.parseFloat(c.attr("step")||1),s=a('<div class="ui-slider '+f+" ui-btn-down-"+g+' ui-btn-corner-all" role="application"></div>'),w=a('<a href="#" class="ui-slider-handle"></a>').appendTo(s).buttonMarkup({corners:true,theme:b,shadow:true}).attr({role:"slider","aria-valuemin":j,"aria-valuemax":p,"aria-valuenow":k(),
"aria-valuetext":k(),title:k(),"aria-labelledby":h});a.extend(this,{slider:s,handle:w,dragging:false,beforeStart:null});if(e=="select"){s.wrapInner('<div class="ui-slider-inneroffset"></div>');c.find("option");c.find("option").each(function(m){var r=m==0?"b":"a",o=m==0?"right":"left";m=m==0?" ui-btn-down-"+g:" ui-btn-active";a('<div class="ui-slider-labelbg ui-slider-labelbg-'+r+m+" ui-btn-corner-"+o+'"></div>').prependTo(s);a('<span class="ui-slider-label ui-slider-label-'+r+m+" ui-btn-corner-"+
...
function(){if(d.dragging){d.dragging=false;if(e==="select"){if(d.beforeStart===c[0].selectedIndex)d.refresh(d.beforeStart===0?1:0);var m=k();m=Math.round(m/(p-j)*100);w.addClass("ui-slider-handle-snapping").css("left",m+"%").animationComplete(function(){w.removeClass("ui-slider-handle-snapping")})}return false}});s.insertAfter(c);this.handle.bind("touchstart mousedown",function(){a(this).focus()});this.handle.bind("keydown",function(m){var r=k();if(!d.options.disabled){switch(m.keyCode){case a.mobile.keyCode.HOME:case a.mobile.keyCode.END:case a.mobile.keyCode.PAGE_UP:case a.mobile.keyCode.PAGE_DOWN:case a.mobile.keyCode.UP:case a.mobile.keyCode.RIGHT:case a.mobile.keyCode.DOWN:case a.mobile.keyCode.LEFT:m.preventDefault();
if(!d._keySliding){d._keySliding=true;a(this).addClass("ui-state-active")}}switch(m.keyCode){case a.mobile.keyCode.HOME:d.refresh(j);break;case a.mobile.keyCode.END:d.refresh(p);break;case a.mobile.keyCode.PAGE_UP:case a.mobile.keyCode.UP:case a.mobile.keyCode.RIGHT:d.refresh(r+q);break;case a.mobile.keyCode.PAGE_DOWN:case a.mobile.keyCode.DOWN:case a.mobile.keyCode.LEFT:d.refresh(r-q)}}}).keyup(function(){if(d._keySliding){d._keySliding=false;a(this).removeClass("ui-state-active")}});this.refresh()},
...
e.first().attr("tabindex","0");e.each(function(h){var k=a(this),j="ui-li";if(!(!d&&k.hasClass("ui-li"))){var p=k.data("theme")||c.theme,q=k.find("a");if(q.length){var s=k.data("icon");k.buttonMarkup({wrapperEls:"div",shadow:false,corners:false,iconpos:"right",icon:q.length>1||s===false?false:s||"arrow-r",theme:p});q.first().addClass("ui-link-inherit");if(q.length>1){j+=" ui-li-has-alt";q=q.last();s=f.data("splittheme")||q.data("theme")||c.splitTheme;q.appendTo(k).attr("title",q.text()).addClass("ui-li-link-alt").empty().buttonMarkup({shadow:false,
corners:false,theme:p,icon:false,iconpos:false}).find(".ui-btn-inner").append(a("<span>").buttonMarkup({shadow:true,corners:true,theme:s,iconpos:"notext",icon:f.data("spliticon")||q.data("icon")||c.splitIcon}))}}else if(k.data("role")==="list-divider"){j+=" ui-li-divider ui-btn ui-bar-"+g;k.attr("role","heading");if(i)i=1}else j+=" ui-li-static ui-btn-up-"+p;if(c.inset){if(h===0){j+=" ui-corner-top";k.add(k.find(".ui-btn-inner")).find(".ui-li-link-alt").addClass("ui-corner-tr").end().find(".ui-li-thumb").addClass("ui-corner-tl");
...
"-")},_createSubPages:function(){var d=this.element,c=d.closest(".ui-page"),f=c.data("url"),b=this.options,g=this,e=c.find("[data-role='footer']").data("id");a(d.find("ul, ol").toArray().reverse()).each(function(i){var h=a(this),k=h.parent(),j=a.trim(k.contents()[0].nodeValue)||k.find("a:first").text();i=f+"&"+a.mobile.subPageUrlKey+"="+g._idStringEscape(j+" "+i);var p=h.data("theme")||b.theme,q=h.data("counttheme")||d.data("counttheme")||b.countTheme;h.wrap("<div data-role='page'><div data-role='content'></div></div>").parent().before("<div data-role='header' data-theme='"+
b.headerTheme+"'><div class='ui-title'>"+j+"</div></div>").after(e?a("<div>",{"data-role":"footer","data-id":e,"class":"ui-footer-duplicate"}):"").parent().attr({"data-url":i,"data-theme":p,"data-count-theme":q}).appendTo(a.mobile.pageContainer).page();h=k.find("a:first");h.length||(h=a("<a></a>").html(j).prependTo(k.empty()));h.attr("href","#"+i)}).listview()}})})(jQuery);
...
(function(a,d,c){var f=a("html"),b=a("head"),g=a(d);a(d.document).trigger("mobileinit");if(a.mobile.gradeA()){f.addClass("ui-mobile ui-mobile-rendering");a.mobile.metaViewportContent&&a("<meta>",{name:"viewport",content:a.mobile.metaViewportContent}).prependTo(b);var e=a.mobile.loadingMessage?a("<div class='ui-loader ui-body-a ui-corner-all'><span class='ui-icon ui-icon-loading spin'></span><h1>"+a.mobile.loadingMessage+"</h1></div>"):c;a.extend(a.mobile,{pageLoading:function(i){if(i)f.removeClass("ui-loading");
else{if(a.mobile.loadingMessage){i=a("."+a.mobile.activeBtnClass).first();e.appendTo(a.mobile.pageContainer).css({top:a.support.scrollTop&&a(d).scrollTop()+a(d).height()/2||i.length&&i.offset().top||100})}f.addClass("ui-loading")}},initializePage:function(){var i=a("[data-role='page']");i.add("[data-role='dialog']").each(function(){var h=a(this);h.data("url")||h.attr("data-url",h.attr("id"))});a.mobile.firstPage=i.first();a.mobile.pageContainer=i.first().parent().addClass("ui-mobile-viewport");a.mobile.pageLoading();
!a.mobile.hashListeningEnabled||!a.mobile.path.stripHash(location.hash)?a.mobile.changePage(a.mobile.firstPage,false,true,false,true):g.trigger("hashchange",[true])}});a(a.mobile.initializePage);g.load(a.mobile.silentScroll)}})(jQuery,this);
